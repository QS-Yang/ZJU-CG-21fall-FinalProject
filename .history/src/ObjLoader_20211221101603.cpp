...
ObjLoader::ObjLoader(string filename, Loader loader)
{
	...
	while (getline(file, line))
	{
		if (line.substr(0,2) == "v ")
		{
            ...
            for(int i = 0; i < 3; i++) {
                int vindex = stoi(words[i * 3 + 1]) >= 0 ? (stoi(words[i * 3 + 1]) - 1) : (stoi(words[i * 3 + 1]) + v.size());
                int vtindex = stoi(words[i * 3 + 2]) >= 0 ? (stoi(words[i * 3 + 2]) - 1) : (stoi(words[i * 3 + 2]) + vt.size());
                int vnindex = stoi(words[i * 3 + 3]) >= 0 ? (stoi(words[i * 3 + 3]) - 1) : (stoi(words[i * 3 + 3]) + vn.size());
